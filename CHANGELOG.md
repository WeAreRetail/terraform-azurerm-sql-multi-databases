...
- Ignore changes to `enclave_type` so AzureRM v5 does not replace existing databases when the enclave setting is not managed by the module.

## [3.1.2] - 2026-06-02

### Other

- Allow Azure AD provider above 2.0.0.

## [3.1.1] - 2026-03-30

### Fix

- A problem  in the condition lead  to always create an unmanaged named replica.

## [3.1.0] - 2026-03-30

### Improvement

- Add `named_replica_ignore_capacity_changes` per-database boolean to create a Hyperscale named replica while letting `sku_name` and `read_replica_count` drift outside of Terraform management.
...
### Breaking Changes
- Update interface to limit named replicas to targetted databases (no project impacted).

## [2.3.0] - 2026-03-23

### Improvement

- Add named replicas and allow HA replicas.
